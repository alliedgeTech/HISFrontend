...
function SearchRegistration({
  ...
  const [SearchRegistrationTempValue, setSearchRegistrationTempValue] =
    useState(false);
  ...
  const searchRegistration = async () => {
    ...
    const data = await ApiManager.get(
      `admin/frontOffice/registration/d/t/${watchSearchBy}?searchValue=${SearchRegistrationTempValue}`
    );
  ...
  const searchRegistrationByDate = async () => {
    setLoading(true);
    const data = await ApiManager.get(
      `admin/frontOffice/registration/d/t/${watchSearchBy}?searchValue=${dayjs(
        DateTempValue?.$d
  ...
  const searchRegistrationByMobileNo = async () => {
    setLoading(true);
    const data = await ApiManager.get(
      `admin/frontOffice/registration/m/${SearchRegistrationTempValue}`
    );
  ...
  const submitSearchData = async (data) => {
    ...
    setSomeSearchDataInForm({
      ...
      email: data?.searchValue?.email,
      _id: data?.searchValue?._id,
    });
    closeTheSearchModal();
  };
  ...
  return (
    ...
    >
      <Box component="form" onSubmit={handleSubmit(submitSearchData)} p={3}>
        <Grid
          container
          spacing={{ md: 3, xs: 2 }}
              ...
              render={({ field, fieldState: { error } }) => {
                ...
                return (
                  <Autocomplete
                    ...
                    getOptionLabel={(op) =>
                      `${op?.doctor?.userName} / ${op?.pationName} / ${
                        op?.city?.cityName
                      } / ${op?.dob?.slice(0, 10)} / ${op?.mobileNo}`
                    }
                  />
                );
              }}></Controller>
